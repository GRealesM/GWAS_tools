#########################################
## DATASET PROJECTION ONTO THE MILMETACYTOKINE BASIS
#########################################

# Introduction: This code is meant to project our processed datasets (previously reduced) onto the milmetacytokine basis.
...
projected.basis <- readRDS("~/rds/rds-cew54-basis/03-Bases/milmetacytokine_basis/milmetacytokine_basis_trait_projection.RDS")
projected.basis[, `:=` (Var.Delta=0, z=0, P=NA)]
setnames(projected.basis, c("delta", "trait"), c("Delta", "Trait"))

files_to_project  <- dir("../milmetacytokine_basis/reduced_datasets/", pattern = ".tsv")
...
projected.table <- lapply(files_to_project, function(file){
	...
	ss.file <- file.path("~/rds/rds-cew54-basis/03-Bases/milmetacytokine_basis/reduced_datasets", file)
...
version  <- 1
projtablename  <- paste("~/rds/rds-cew54-basis/03-Bases/Projections/Projection_milmetacytokine_basis_", date, "-v",version, ".tsv", sep="")
qctablename  <- paste("~/rds/rds-cew54-basis/03-Bases/Projections/QC_milmetacytokine_basis_", date, "-v",version, ".tsv", sep="")
