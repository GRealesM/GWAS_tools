...
library(Matrix)
library(magrittr)
load("~/rds/rds-cew54-basis/03-Bases/cytokine13_basis/cytokine13-basis-sparse-1.0.RData")

# We load the traits that were used to create the basis first, then we'll append the rest of traits to that table
projected.basis <- readRDS("~/rds/rds-cew54-basis/03-Bases/cytokine13_basis/cytokine13_basis_trait_projection.RDS")
projected.basis[, `:=` (Var.Delta=0, z=0, P=NA)]
setnames(projected.basis, c("delta", "trait"), c("Delta", "Trait"))

files_to_project  <- dir("~/rds/rds-cew54-basis/03-Bases/cytokine13_basis/reduced_datasets/", pattern = ".tsv")
...
projected.table <- lapply(files_to_project, function(file){
	...
	ss.file <- file.path("~/rds/rds-cew54-basis/03-Bases/cytokine13_basis/reduced_datasets", file)
...
version  <- 1
projtablename  <- paste("~/rds/rds-cew54-basis/03-Bases/Projections/Projection_cytokine13_basis_", date, "-v",version, ".tsv", sep="")
qctablename  <- paste("~/rds/rds-cew54-basis/03-Bases/Projections/QC_cytokine13_basis_", date, "-v",version, ".tsv", sep="")
