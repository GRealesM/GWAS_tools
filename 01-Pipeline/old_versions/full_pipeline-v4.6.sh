...
### For Hoglund processing (A1 = REF, A2 = ALT) I modified allele assignation accordingly, please check what's the case in your files prior to pipelining them.

 echo Reheading...
 	head -n1 tmp_fs_file.tsv | awk -F'\t' '{
 	gsub(/\<Log10p\>/,"LOG10P");
 	gsub(/\<_-log10_p-value\>/,"-LOG10P");
 	gsub(/\<effect_allele\>|\<Effect_Allele\>|\<EffectAllele\>|\<A1_effect\>|\<RISK_ALLELE\>|\<EA\>|\<Risk_Allele\>|\<EFFECT_ALLELE\>|\<Alt\>|\<A2\>|\<a1\>|\<alt\>|\<Allele2\>/,"ALT");
 	gsub(/\<OtherAllele\>|\<reference_allele\>|\<Ref_Allele\>|\<OTHER_ALLELE\>|\<other_allele\>|\<A2_other\>|\<NEA\>|\<Ref_Allele\>|\<Ref\>|\<A1\>|\<a2\>|\<ref\>|\<Allele1\>/,"REF");
 # Caution! Sometimes "other_allele" means effect allele, check papers prior to run the script, and pre-rename accordingly.
 	gsub(/\<Beta\>|\<beta\>|\<Effect\>|\<effect\>|\<EFFECT\>|\<beta_SNP_add\>|\<EFFECT_ALT\>|\<effB\>/,"BETA");
 	...
 	gsub(/\<p_value\>|\<P.value\>|\<pvalue\>|\<P-value\>|\<pval\>|\<p.value\>|\<Pval\>|\<PVALUE\>|\<Pvalue\>|\<P_VALUE\>|\<P-val\>|\<p\>|\<All.p.value\>|\<P_value\>|\<p-value\>|\<GC-adjusted_P_\>|\<Chi-Squared__P\>|\<P1df\>/,"P");
 	gsub(/\<standard_error\>|\<StdErr\>|\<stderr\>|\<sebeta_SNP_add\>|\<se\>|\<STDERR\>|\<sebeta\>|\<se_effB\>/,"SE");
 	gsub(/\<Rsq\>/,"RSQ");
...
if [[ "$minBETA" == 0 && 1 -eq $(echo  "$NABETA > $maxNA" | bc -l) ]]; then
	if [[ "$minOR" == 0 ]]; then
		...
		if [[ 1 -eq $(echo  "$NAOR > $maxNA" | bc -l) ]]; then
			...
			cut -f"$BETACOL" --complement tmp_schecked.tsv | awk -v orcol="$ORCOL" 'BEGIN{FS=OFS="\t"} {print $0,log($orcol)}' | sed -e 's/^M//' -e '1s/-inf/BETA/' > tmp.tsv && mv tmp.tsv tmp_schecked.tsv
		fi
	fi
fi
...
# Note: 3 = hg18, 5 = hg19, and 7 = hg38
 ...
 join -a1 -e'NA' -t $'\t' --nocheck-order -o auto "${FILEBASENAME}"-lo-output2.bed tmp_formerging1.tsv | awk '!seen[$0]++' | gzip  > "${FILEBASENAME}"-hg38.tsv.gz
