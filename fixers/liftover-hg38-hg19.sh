...
do
 ...
 awk -v snpidcol="$SNPIDCOL" -v chrcol="$CHRCOL" -v bpcol="$BPCOL" 'BEGIN{FS=OFS="\t"}{print "chr"$chrcol, $bpcol, (($bpcol + 1)), $snpidcol }' workingfile.tsv | tail -n+2 > ${FILEBASENAME}.bed

 liftOver "${FILEBASENAME}".bed hg38ToHg19.over.chain.gz "${FILEBASENAME}"-lo-output.bed "${FILEBASENAME}"-unlifted.bed
