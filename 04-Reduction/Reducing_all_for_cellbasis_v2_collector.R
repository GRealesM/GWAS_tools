...
# NOTE: This is a collector version, meaning that instead of using batches of files to process in a for loop, slurm will provide 
# the very number of the files that failed in the previous run, so only one file will be generated per job.
# This is intended to prevent the reprocessing of the files already processed, and cut queue time.
