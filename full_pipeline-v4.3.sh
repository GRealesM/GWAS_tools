...
### NOTE: I TEMPORARILY ADDED A1 to be replaced by ALT and A2 by REF for Enroth files processing. This may not be the case for other files, so remember to remove them later!
 ...
 echo Reheading...
 	...
 	gsub(/\<effect_allele\>|\<Effect_Allele\>|\<EffectAllele\>|\<A1_effect\>|\<RISK_ALLELE\>|\<EA\>|\<Risk_Allele\>|\<EFFECT_ALLELE\>|\<Alt\>|\<A1\>|\<alt\>/,"ALT");
 	gsub(/\<OtherAllele\>|\<reference_allele\>|\<Ref_Allele\>|\<OTHER_ALLELE\>|\<other_allele\>|\<A2_other\>|\<NEA\>|\<Ref_Allele\>|\<Ref\>|\<A2\>|\<ref\>/,"REF");
 # Caution! Sometimes "other_allele" means effect allele, check papers prior to run the script, and pre-rename accordingly.
 	...
 	gsub(/\<Chr\>|\<chromosome\>|\<Chromosome\>|\<chr\>|\<Chr_ID\>|\<hg18chr\>|\<CHROMOSOME\>|#chrom\>|\<chrom\>/,"CHR");
