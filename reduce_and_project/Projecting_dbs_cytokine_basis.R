...
projected.table <- lapply(files_to_project, function(file){
	...
	ss.file <- file.path("../cytokine_basis/reduced_datasets", file)
...
version  <- 1
projtablename  <- paste("Projection_cytokine_basis_", date, "-v",version, ".tsv", sep="")
qctablename  <- paste("QC_cytokine_basis_", date, "-v",version, ".tsv", sep="")
