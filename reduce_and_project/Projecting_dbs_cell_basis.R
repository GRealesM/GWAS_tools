...
projected.table <- lapply(files_to_project, function(file){
	...
	ss.file <- file.path("../cell_basis/reduced_datasets", file)
...
version  <- 1
projtablename  <- paste("Projection_cell_basis_", date, "-v",version, ".tsv", sep="")
qctablename  <- paste("QC_cell_basis_", date, "-v",version, ".tsv", sep="")
