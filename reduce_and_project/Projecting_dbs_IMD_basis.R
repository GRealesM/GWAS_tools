...
build_dict <- fread("../IMD_basis/Manifest_build_translator.tsv")
...
files_to_project  <- dir("../IMD_basis/reduced_datasets/", pattern = ".tsv")
...
projected.table <- lapply(files_to_project, function(file){
	...
	ss.file <- file.path("../IMD_basis/reduced_datasets", file)
...
version  <- 1
projtablename  <- paste("Projection_IMD_basis_", date, "-v",version, ".tsv", sep="")
qctablename  <- paste("QC_IMD_basis_", date, "-v",version, ".tsv", sep="")
